            ...
            return (
              <div key={s.id} id={s.id} className="scroll-mt-20">
                <StageCard
                  ...
                  onRemoveTask={(t) => removeTask.mutate(s.tasks.find((x) => x.id === t.id)!)}
                  headerExtra={
                    <>
                ...
                >
                  <AddTask rooms={rooms ?? []} onAdd={(name, room_id) => saveTask.mutate({ stage_id: s.id, name, room_id, sort_order: s.tasks.length + 1 })} />
                  {fromChecklist !== null && fromChecklist !== s.progress && s.status !== "done" && (
                  ...
                  {s.client_note && <p className="mt-3 text-sm text-muted-foreground">Note for client: {s.client_note}</p>}
                </StageCard>
              </div>
            );
          })}
