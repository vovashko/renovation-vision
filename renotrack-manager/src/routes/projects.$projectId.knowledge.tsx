...
import { Field, FormSheet, VisibleSwitch } from "@/components/form-sheet";
            ...
            <article key={k.id} className={`rounded-xl border bg-card p-5 shadow-[var(--shadow-soft)] ${k.is_visible ? "" : "border-dashed"}`}>
              ...
              {k.tags.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {k.tags.map((t) => <span key={t} className="rounded-full bg-muted px-2.5 py-0.5 text-xs">{t}</span>)}
                </div>
              )}
...
function KnowledgeSheet({ projectId, entry, onClose }: { projectId: string; entry: Knowledge | "new" | null; onClose: () => void }) {
  ...
  if (key !== lastKey) {
    setLastKey(key);
    setForm(entry === null ? null : isNew ? { title: "", content: "", tags: "", is_visible: true } : { title: entry.title, content: entry.content, tags: entry.tags.join(", "), is_visible: entry.is_visible });
  }
  ...
  return (
    <FormSheet open={entry !== null} onOpenChange={(v) => !v && onClose()} title={isNew ? "Add knowledge" : "Edit knowledge"} description="Write it the way you'd explain it to the client.">
      {form && (
        <form
          className="space-y-4"
          onSubmit={(e) => {
            e.preventDefault();
            save.mutate(
              { ...(isNew ? {} : { id: (entry as Knowledge).id }), title: form.title.trim(), content: form.content.trim(), is_visible: form.is_visible, tags: form.tags.split(",").map((t) => t.trim()).filter(Boolean) },
              { onSuccess: onClose },
            );
          }}
        >
          <Field id="kn-title" label="Topic or question"><Input id="kn-title" required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="h-11" /></Field>
          <Field id="kn-content" label="Answer"><Textarea id="kn-content" required value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} className="min-h-32" /></Field>
          <Field id="kn-tags" label="Tags" hint="Comma separated, e.g. kitchen, delivery"><Input id="kn-tags" value={form.tags} onChange={(e) => setForm({ ...form, tags: e.target.value })} className="h-11" /></Field>
          <VisibleSwitch id="kn-visible" checked={form.is_visible} onChange={(v) => setForm({ ...form, is_visible: v })} label="Assistant may use this with the client" />
