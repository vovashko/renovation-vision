        ...
        <>
          <div className="mt-6">
            <FloorPlan
              ...
              detail={active ? <RoomEditor key={active.id + active.status + active.progress} projectId={projectId} room={active} openTasks={openTasks(active.id).map((t) => t.name)} /> : undefined}
            />
          </div>
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {rooms.map((r) => (
              <RoomCard key={r.id} name={r.name} status={r.status} progress={r.progress} active={r.id === activeId} muted={!r.is_visible} onClick={() => setActiveId(r.id)}>
                {!r.is_visible && <div className="mt-2"><VisibilityBadge visible={false} /></div>}
              </RoomCard>
            ))}
          </div>
        </>
