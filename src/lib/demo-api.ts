...
export const DEMO_CLIENT = { id: SARAH, full_name: "Sarah Bennett", email: "sarah@renovision.demo" };
...
export const demoApi: Api = {
  ...
  listRooms: () => wait(db.rooms.filter((r) => !isClient() || r.is_visible).sort((a, b) => a.sort_order - b.sort_order)),
  ...
  async sendMessage(_id, body, file) {
    ...
    db.messages.push({ id: uid(), project_id: PID, sender_id: demoUser().id, body, attachment_path, created_at: now() });
  ...
  joinPresence(_id, me, onChange) {
    const t = setTimeout(() => onChange([me.id, isClient() ? DEMO_USER.id : SARAH]), 300);
  ...
  listKnowledge: () => wait(db.knowledge.filter((k) => !isClient() || k.is_visible)),
