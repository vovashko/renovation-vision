  ...
  return (
    <div className="mx-auto w-full max-w-6xl">
      <PageHeader
        ...
        description={isManager ? "Renders show the client how each room will look when finished." : "How each room will look when it is finished."}
        actions={isManager && <Button onClick={() => setEditing("new")} className="min-h-11 gap-2"><ImagePlus className="h-4 w-4" /> Add render</Button>}
      />
      ...
      {renders.length === 0 && <EmptyState className="mt-6" icon={Palette} title="No renders yet" text={isManager ? "Add the designer's renders so the client can see the finished look." : "Design renders will appear here once your designer shares them."} />}
          ...
          <section key={g.room?.id ?? "none"} aria-label={g.room?.name ?? "No room"}>
            ...
            {g.items.length === 0 ? (
              <div className="flex items-center gap-3 rounded-xl border border-dashed bg-card p-5 text-sm text-muted-foreground">
                <Palette className="h-5 w-5 shrink-0" /> {isManager ? "The client sees " : ""}"Renders for {g.room?.name} are still being prepared by the designer."
              </div>
            ) : (
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {g.items.map((r) => (
                  <article key={r.id} className={`overflow-hidden rounded-xl border bg-card shadow-[var(--shadow-soft)] ${!isManager || r.is_visible ? "" : "border-dashed"}`}>
                    <img src={r.url} alt={r.alt} loading="lazy" width={1024} height={768} className="aspect-[4/3] w-full object-cover" />
                    <div className="p-4">
                      <div className="font-medium">{r.title}</div>
                      <p className="mt-1 text-sm text-muted-foreground">{r.description}</p>
                      {isManager && <div className="mt-3 flex items-center justify-between gap-2">
