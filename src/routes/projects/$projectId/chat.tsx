...
function ChatPage() {
  ...
  return (
    <div
      className={`-mx-4 -my-4 flex ${isManager ? "h-[calc(100dvh-3.5rem)]" : "h-[calc(100dvh-7.5rem-env(safe-area-inset-bottom))]"} flex-col overflow-hidden bg-card md:mx-auto md:my-0 md:h-[calc(100dvh-8rem)] md:w-full md:max-w-3xl md:rounded-2xl md:border md:shadow-[var(--shadow-elegant)]`}
    >
      {!isManager && (
          ...
          <div role="tablist" aria-label="Chat mode" className="flex gap-1 rounded-full bg-muted p-1">
            {(
              ...
              <button
                ...
                aria-selected={tab === k}
                onClick={() => setTab(k)}
                className={`flex min-h-11 flex-1 items-center justify-center gap-2 rounded-full text-sm font-medium transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none ${tab === k ? "bg-card text-foreground shadow-[var(--shadow-soft)]" : "text-muted-foreground"}`}
              >
