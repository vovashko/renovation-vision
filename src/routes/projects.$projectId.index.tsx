...
function Overview() {
  ...
  const warnings = isManager ? findInconsistencies(project, stages, rooms) : [];
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-8">
      <ProjectHeaderCard
        ...
        actions={isManager && <Button variant="outline" onClick={() => setEditing(true)} className="min-h-11 gap-2"><Pencil className="h-4 w-4" /> Edit project details</Button>}
      />
      ...
      {project.schedule_note && (
        <p className="-mt-4 rounded-xl border bg-card p-4 text-sm text-muted-foreground shadow-[var(--shadow-soft)]">
          <span className="font-medium text-foreground">{isManager ? "Schedule note for the client: " : "Schedule note: "}</span>{project.schedule_note}
        </p>
      )}
      ...
      <section>
        <div className="mb-3 flex items-end justify-between">
          ...
          <Link to="/projects/$projectId/stages" params={{ projectId }} className="text-sm text-primary hover:underline">{isManager ? "Edit stages →" : "All stages →"}</Link>
        </div>
      ...
      <section>
        <div className="mb-3 flex items-end justify-between">
          ...
          <Link to="/projects/$projectId/plan" params={{ projectId }} className="text-sm text-primary hover:underline">{isManager ? "Edit rooms →" : "Open plan →"}</Link>
        </div>
        <FloorPlan
      ...
      {isManager && <ProjectDetailsSheet project={project} open={editing} onOpenChange={setEditing} />}
    </div>
  );
}
