  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex-1 space-y-4 overflow-y-auto p-4" aria-live="polite">
        {messages.length === 0 && (
          <div className="mx-auto max-w-md py-6 text-center">
            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[image:var(--gradient-primary)] text-primary-foreground"><Bot className="h-6 w-6" /></div>
        ...
        {messages.map((m) =>
          m.role === "user" ? (
            <div key={m.id} className="flex justify-end">
              <div className="max-w-[85%] rounded-2xl rounded-br-sm bg-[image:var(--gradient-primary)] px-4 py-2 text-sm text-primary-foreground">{m.text}</div>
            </div>
          ) : (
      ...
      <div className="border-t bg-background/60 backdrop-blur">
        ...
        <form className="flex items-center gap-2 p-3" onSubmit={(e) => { e.preventDefault(); ask(input); }}>
          {messages.length > 0 && (
            <button type="button" onClick={() => setMessages([])} aria-label="Clear chat" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-muted-foreground hover:bg-muted">
