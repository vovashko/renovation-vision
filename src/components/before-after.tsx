...
export function BeforeAfter({ before, after, label }: { before: string; after: string; label: string }) {
  ...
  return (
    <div className="relative aspect-[4/3] w-full select-none overflow-hidden rounded-xl border bg-muted shadow-[var(--shadow-soft)]">
      ...
      <span className="absolute left-3 top-3 rounded-full bg-foreground/70 px-2.5 py-1 text-xs font-medium text-background">Now</span>
      <span className="absolute right-3 top-3 rounded-full bg-primary px-2.5 py-1 text-xs font-medium text-primary-foreground">Planned</span>
      <input
        type="range"
        min={0}
        max={100}
